...
function OrdersComponent({ orders }: { orders: MY_ORDERS_QUERYResult }) {
  const [selectedOrder, setSelectedOrders] = useState<MY_ORDERS_QUERYResult[number] | null>(null);
  ...
  return (
    <>
              ...
              <TooltipTrigger asChild>
                ...
                onClick={()=> setSelectedOrders(order)}>
                  ...
                  <TableCell className="hidden md:table-cell">
                    {order?.orderDate &&
                      format(new Date(order.orderDate), "dd/MM/yyy")}
                  </TableCell>
      ...
      <OrderDetailsDialog order={selectedOrder} isOpen={!!selectedOrder} onClose={()=>setSelectedOrders(null)}/>
    </>
  );
}
